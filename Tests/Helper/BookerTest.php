...
namespace Slad\BookingBundle\Tests\Helper;

use Slad\BookingBundle\Tests\Fixtures\ORM\Entity\Booking;
use Symfony\Bundle\FrameworkBundle\Test\WebTestCase;
use Slad\BookingBundle\Helper\Booker;
...
class BookerTest extends WebTestCase
{
    ...
    public function setUp()
    {
        ...
        $this->item = $this->container->get('doctrine')
            ->getRepository('Slad\BookingBundle\Tests\Fixtures\ORM\Entity\BookableItem')
            ->findOneBy(array());
    }

    public function testShouldBeConstructedWithNeededArguments()
    {
        new Booker('Slad\BookingBundle\Tests\Fixtures\ORM\Entity\Booking',
    ...
    public function testKernelShouldContainBookerService()
    {
        ...
        $this->assertInstanceOf('Slad\BookingBundle\Helper\Booker', $this->container->get('booker'));
    }
    ...
    public function testWhereAvailableForPeriod($start, $end)
    {
        ...
        $this->assertEquals('SELECT i FROM Slad\BookingBundle\Tests\Fixtures\ORM\Entity\BookableItem i LEFT JOIN bookings b WHERE (b.start >= :start AND b.end <= :end) OR (b.start <= :start AND b.end >= :end) OR (b.start <= :start AND b.end >= :end AND b.start <= :end) OR (b.start >= :start AND b.end <= :end AND b.end >= :start)', $dql);
    }
    ...
    public function testWhereIsAvailableForSingleDate($start)
    {
        ...
        $this->assertEquals('SELECT i FROM Slad\BookingBundle\Tests\Fixtures\ORM\Entity\BookableItem i LEFT JOIN bookings b WHERE b.start >= :date AND b.end <= :date', $dql);
    }
